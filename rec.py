...
def ask_openai(system_prompt: str, user_prompt: str, model="ax4", temperature=0.7):
...
def main():
    ...
    if customInfo["양식"] == "편지형":
        ...
        for stu in student_infos:
            ...
            #정보 중심 입력값 + 피드백 중심 입력값 통합 템플릿 처리
            ...
            message = INFO_TMPL.render(
                ...
                test = merged.get("테스트"),
                cumulative_test = merged.get("누적_테스트"),
                attendance = merged.get("학생_피드백", {}).get("출결"),
                stu_fd = feedback
            )
