...
processed_keys_daily = set(["daily/lessonInfo/수업/수업_난이도/상", "daily/lessonInfo/수업/수업_난이도/중상", "daily/lessonInfo/수업/수업_난이도/중",  
    ...
    if isinstance(example_dict[key], dict):
        ...
        if type(example_dict[key]) != int and example_dict[key] != None:
            ...
            if path in processed_keys: 
                ...
                set_replace_dict(data, path.split("/")[1:], "replace")
...
def main():
    ...
    if customInfo["피드백_형식"] == "오늘 수업 피드백":
        ...
        for key in keys:
            rec_explore_dict(data, key, "daily/" + key, processed_keys_daily)

    elif customInfo["피드백_형식"] == "이번 주 피드백":

        for key in keys:
            rec_explore_dict(data, key, "weekly/" + key, processed_keys_weekly)

    elif customInfo["피드백_형식"] == "이번 달 피드백":

        for key in keys:
            rec_explore_dict(data, key, "monthly/" + key, processed_keys_monthly)
